...
impl GitSegment {
    ...
    fn get_git_info_from_command(&self, working_dir: &str) -> Option<GitInfo> {
...
impl Segment for GitSegment {
    fn collect(&self, input: &QwenInput) -> Option<SegmentData> {
        // Prefer Qwen Code's pre-computed git branch, fall back to git command
        let git_info = if let Some(ref qwen_git) = input.git {
            if !qwen_git.branch.is_empty() {
                // Use Qwen Code's branch info, but still get status from git command
                let working_dir = &input.workspace.current_dir;
                let status = if !working_dir.is_empty() {
                    self.get_status(working_dir)
                } else {
                    GitStatus::Clean
                };
                let (ahead, behind) = if !working_dir.is_empty() {
                    self.get_ahead_behind(working_dir)
                } else {
                    (0, 0)
                };
                let sha = if self.show_sha && !working_dir.is_empty() {
                    self.get_sha(working_dir)
                } else {
                    None
                };
                GitInfo {
                    branch: qwen_git.branch.clone(),
                    status,
                    ahead,
                    behind,
                    sha,
                ...
                self.get_git_info_from_command(&input.workspace.current_dir)?
            }
        } else {
            self.get_git_info_from_command(&input.workspace.current_dir)?
        };
